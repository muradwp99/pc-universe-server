...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.lkutvwc.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
    ...
    const carPartsCollection = client.db('perlictools').collection('parts')
    const ordersCollection = client.db('perlictools').collection('orders')
    const paymentCollection = client.db('perlictools').collection('payments')
    const reviewCollection = client.db('perlictools').collection('reviews')
    const userCollection = client.db('perlictools').collection('users')
    try {
